...
const Category = () => {
  ...
  useEffect(() => {
    axios
      .get('https://panda-snack-server.vercel.app/categorys/categories', {
        headers: {
  ...
  useEffect(() => {
    if (selectedCategory) {
      axios
        .get(`https://panda-snack-server.vercel.app/meals/mealsbycat/${selectedCategory._id}`, {
          headers: {
            Authorization: 'JWT Mft26100$$',
            'Content-Type': 'multipart/form-data',
            Accept : '*/*',
    ...
    <div className='flex flex-wrap items-center justify-center mt-10 flex-grow md:mx-20 my-10'>
        ...
        mealsByCat.map((item) => (
            <div key={item._id} className='md:w-1/3 dark:bg-lime-900 my-1 py-1 px-2 sm:w-1/2 flex items-center border-2 border-lime-300 rounded-md mx-2 md:mx-1 flex-col h-[425px]'>
                <img src={`https://panda-snack-server.vercel.app/meal_picture/${item.Mealpicture}`} alt={item.name} className='hover:rotate-45 transition-all duration-500' width={250}/>
